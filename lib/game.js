...
var Game = function() {
    ...
    var Winner = function(winnerFile) {
        ...
        self.addWinner = function(prizeId, employees) {
            ...
            _.each(employees, function(employee) {
                list.push({depart: employee.depart, title:employee.title, name:employee.name});
            });
        ...
        self.removeWinner = function(prizeId, employees) {
            ...
            _.each(employees, function(employee) {
                var removed = _.remove(list, function(w) {
                   return w.depart == employee.depart && w.title == employee.title;
                });
    ...
    self.drawPrize = function(prizeId, count) {
        ...
        var toDrawVar = toDraw;

        for (var i = 0; (toDrawVar >0 &&  availables.length >0 ); i++) {
            ...
            var removed = _.remove(self.candidates[prize.group], function(emp) {
                return emp.depart == winner.depart && emp.title == winner.title;
            });
            toDrawVar--;
        ...
        _.each(list, function(w) { logger.debug(w.depart + ':' + w.name + ':' + w.title);});
                ...
                if (clist) {
                    _.each(records, function(record) {
                        ...
                        var removed = _.remove(clist, function(employee) {
                            return employee.depart == record.depart && employee.title == record.title;
                        });
        ...
        _.each(prizeGroup, function(prizeList, key) {
            ...
            _.each(prizeList, function(prize) {
                ...
                _.each(winners, function(winner) {
                    ...
                    //
                    var dup = _.find(winnerList, function(winnerT) { return winner.depart == winnerT.depart && winner.title == winnerT.title;});
                    if (dup) {
                        logger.error("INTERNAL ERROR: duplicate in winners = " + winner.depart + ':' + winner.name+ ':' + winner.title);
                    }
                    else {
